...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:print="http://www.sub.uni-goettingen.de/BE/OUS/print" exclude-result-prefixes="xs" version="2.0">
    <xsl:output indent="yes"/>
    <xsl:param name="template" select="'cover-letter'" as="xs:string"/>
                    ...
                    <xsl:analyze-string select=".//text()[not(matches(., '^\s*$', 'sm'))]"
                        ...
                        <xsl:matching-substring>
                            ...
                            <xsl:if test="contains($level, 'Dritte')">
                                <xsl:variable name="adress-raw"
                                    select="replace(regex-group(2), '.*ol@sub.uni-goettingen.de\s*(.+)\s*Nutzernummer.*', '$1', 'sm')" as="xs:string"/>
                                ...
                                <xsl:variable name="date" select="translate(replace(regex-group(2), '.*Rueckgabedatum:\s([\w-]{10}).*', '$1', 'sm'), '-', '.')"
                                    as="xs:string"/>
                                ...
                                <!-- Fix adress -->
                                <xsl:variable name="adress" select="print:fix-umlauts($adress-raw)" as="xs:string"/>
                               
                                <xsl:choose>
                                    <xsl:when test="$template = 'cover-letter'">
                                        <!-- cover letter -->
                                        <fo:block page-break-after="always" font-family="Arial" font-size="12pt">
                                            <fo:block text-align-last="justify">Niedersächsische Staats- und Universitätsbibliothek <fo:leader
                                                    leader-pattern="space"/>Göttingen, <xsl:value-of select="format-dateTime(current-dateTime(), '[D].[M].[Y]')"
                                                />
                                            </fo:block>
                                            <fo:block>SUB - Zentralbibliothek – Leihstelle</fo:block>
                                            <fo:block>Platz der Göttinger Sieben 1</fo:block>
                                            <fo:block text-align-last="justify">37070 Göttingen<fo:leader leader-pattern="space"
                                                />ol@sub.uni-goettingen.de</fo:block>
                                                ...
                                                <fo:list-item>
                                                    ...
                                                    <fo:list-item-body start-indent="9mm">
                                                        <fo:block>Der folgende Band, dessen Leihfrist abgelaufen ist, ist gem. Par. 22 der Benutzungsordnung der
                                                            Niedersächsischen Staats- und Universitätsbibliothek Göttingen vom 18.12.1996 innerhalb von 14 Tagen
                                                            nach Zustellung dieses Bescheides zurückzugeben und die angefallenen Mahngebühren sind zu
                                                            entrichten. Außerdem werden Sie bis zur Rückgabe des Werkes bzw. einer evtl. Ersatzleistung von der
                                                            Ausleihe ausgeschlossen.</fo:block>
                                                    </fo:list-item-body>
                                                </fo:list-item>
                                                <fo:list-item>
                                                    <fo:list-item-label start-indent="4mm">
                                            ...
                                            <fo:block margin-top="0.5cm">Wir bitten Sie, den folgenden Band, dessen Leihfrist abgelaufen ist, umgehend
                                                zurückzugeben: </fo:block>
                                            ...
                                            <fo:block>
                                                <xsl:choose>
                                                    <xsl:when test="$author != ''"> Autor:<fo:leader leader-pattern="space"/><xsl:value-of select="$author"
                                                        /></xsl:when>
                                                    <xsl:otherwise>
                                            ...
                                            <fo:block>SUB Göttingen</fo:block>
                                            <fo:block font-size="11pt">
                                                <fo:footnote>
                                                    <fo:inline/>
                                                    <fo:footnote-body>
                                                        <fo:block keep-together.within-column="always">
                                                            <fo:block>Dieses Schreiben wurde maschinell erstellt und bedarf daher keiner
                                                                Unterschrift.</fo:block>
                                                            <fo:block>Beachten Sie die beiliegenden Hinweise nebst der abschließenden
                                                                Rechtsbehelfsbelehrung.</fo:block>
                                                            <fo:block>Nord LB, BLZ 25050000, Kto.106032725, IBAN: DE49250500000106032725,</fo:block>
                                                            <fo:block>BIC: NOLADE2HXXX, Verwendungszweck:Gebühren/Nutzernummer</fo:block>
                                                        </fo:block>
                                                    </fo:footnote-body>
                                                </fo:footnote>
                                            </fo:block>
    ...
    <xsl:function name="print:fix-umlauts">
        <xsl:param name="str" as="xs:string"/>
        <!--
        <xsl:value-of select="replace(replace($str, 'G_ttingen', 'Göttingen'), 'b~u', 'bäu')"/>
        -->
        <xsl:value-of select="replace(replace($str, '(\w)_(\w)', '$1ö$2'), '(\w)~(\w)', '$1ä$2')"/>
    </xsl:function>
</xsl:stylesheet>
